...
function normalizeTargetType(value) {
  ...
  if (!Object.values(TARGET_TYPES).includes(targetType)) {
    throw badRequest('Loại Sheet phải là ALL hoặc BRANCH');
  }
...
function validateTargetPayload(input = {}, { partial = false } = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'branch_id')) {
    data.branch_id = input.branch_id === undefined || input.branch_id === null || input.branch_id === ''
      ? null
      : parsePositiveInteger(input.branch_id, 'branch_id');
  }
...
async function ensureBranch(branchId, executor = pool) {
  const result = await executor.query('SELECT id FROM branches WHERE id = $1', [branchId]);
...
async function ensureTargetShape(data, current = null) {
  const targetType = data.target_type || current?.target_type;
  const branchId = Object.prototype.hasOwnProperty.call(data, 'branch_id') ? data.branch_id : current?.branch_id;

  if (targetType === TARGET_TYPES.ALL && branchId) {
    throw badRequest('Sheet tổng không được gắn chi nhánh');
  }

  if (targetType === TARGET_TYPES.BRANCH && !branchId) {
    throw badRequest('Sheet chi nhánh phải chọn chi nhánh');
  }

  if (targetType === TARGET_TYPES.BRANCH) {
...
async function listSheetTargets(executor = pool) {
  const result = await executor.query(
    `SELECT sst.id, sst.name, sst.target_type, sst.branch_id, br.name AS branch_name,
            sst.webhook_url, sst.is_active, sst.last_sync_at, sst.last_error, sst.created_at, sst.updated_at
     FROM sheet_sync_targets sst
     LEFT JOIN branches br ON br.id = sst.branch_id
     ORDER BY CASE sst.target_type WHEN 'ALL' THEN 0 ELSE 1 END, br.name ASC, sst.created_at DESC, sst.id DESC`
  );
...
async function createSheetTarget(input = {}) {
  ...
  if (data.target_type === TARGET_TYPES.ALL) {
    data.branch_id = null;
  }

  await ensureTargetShape(data);
  ...
  const result = await pool.query(
    `INSERT INTO sheet_sync_targets (name, target_type, branch_id, webhook_url, is_active)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`,
    [data.name, data.target_type, data.branch_id, data.webhook_url, data.is_active === undefined ? true : data.is_active]
  );
...
async function updateSheetTarget(id, input = {}) {
  ...
  await ensureTargetShape(data, current);
  if (data.webhook_url) {
  ...
  for (const column of ['name', 'target_type', 'branch_id', 'webhook_url', 'is_active']) {
...
function bookingPayload(booking) {
  return {
    ...
    address: booking.branch_name || String(booking.branch_id),
  ...
  const result = await pool.query(
    `SELECT sst.id, sst.name, sst.target_type, sst.branch_id, br.name AS branch_name,
     ...
     WHERE sst.is_active = TRUE
       AND (sst.target_type = 'ALL' OR sst.branch_id = $1)
